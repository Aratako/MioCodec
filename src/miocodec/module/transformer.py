...
_FLASH_CPU_WARNED = False


def _warn_flash_cpu_once():
    global _FLASH_CPU_WARNED
    if _FLASH_CPU_WARNED:
        return
    logger.warning("FlashAttention is installed but input is on CPU; falling back to PyTorch SDPA.")
    _FLASH_CPU_WARNED = True
